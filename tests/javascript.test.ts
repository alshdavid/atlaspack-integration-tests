...
import { evalEsm, evalEsmCb } from "../utils/eval-esm.ts";
import { assertBundles, getBundleData } from "../utils/bundle.ts";
import { concurrency } from "../utils/constants.ts";
import { evalCjs, evalCjsCb } from "../utils/eval-cjs.ts";

const __dirname = path.dirname(url.fileURLToPath(import.meta.url));

describe("javascript", { concurrency }, () => {
  test("does not fail when there is a transitive import of an empty file with export *", async () => {
    const fixture = await Fixture.yaml/*yaml*/ `
      package.json:
        name: "javascript-transitive-import"
        type: "module"
        main: "dist/index.js"
        source: "index.js"
        sideEffects: false
    ...
    const resultEsm = await evalEsm(entries.get("index.js"));

    assert.equal(resultEsm.default, "should not fail");
  });

  test("should produce a basic JS bundle with CommonJS requires", async () => {
    const fixture = await Fixture.yaml/*yaml*/ `
      package.json:
        name: "javascript-cjs-alias"
        type: "module"
        main: "dist/index.js"
        source: "index.js"
        alias:
    ...
    const [outputs, buildEvent] = await fixture.bundle("index.js");
    ...
    const result = await evalEsm(outputs.get("index.js"));
    assert.equal(typeof result.default, "function");
    assert.equal(result.default(), 3);
  });

  test("should support url: imports with CommonJS output", async () => {
    const fixture = await Fixture.yaml/*yaml*/ `
      package.json:
        name: "javascript-cjs-url-import"
        type: "commonjs"
        source: "index.js"
        engines:
          node: ">=10"
    ...
    const [, , bundleGraph] = await fixture.bundle("index.js");
    ...
    const result = await evalCjs<string>(fixture.path("dist", "index.js"));

    const txtBundle = bundleGraph.getBundles().find((b) => b.type === "txt");
  ...
  test("should produce a basic JS bundle with ES6 imports", async function () {
    const fixture = await Fixture.yaml/*yaml*/ `
      package.json:
        name: "javascript-esm-basic-bundle"
        type: "module"
        main: dist/index.js
        source: "index.js"
    ...
    const output = await evalEsm(entries.get("index.js"));
  ...
  test("should ignore unused requires after process.env inlining", async function () {
    // Test changed because env is not replaced when using library bundler
    const fixture = await Fixture.yaml/*yaml*/ `
      package.json:
        name: "javascript-ignore-unused-requires"
        type: "module"

      index.js: |
        if(process.env.ABC === 'a') {
          done(require("./unused.js"));
        } else {
          done("ok");
        }
    ...
    const [entries, , bundle] = await fixture.bundleWithOptions(["index.js"], {
      env: { ABC: "XYZ" },
    });

    assertBundles(bundle, [
      {
        type: "js",
        assets: ["index.js"],
      },
    ...
    const index = entries.get("index.js");
    if (!index) throw new Error("Unable to get entry");

    const contents = await fs.promises.readFile(index, "utf8");
    assert.ok(!contents.includes("unused"));

    const value = await evalEsmCb<string>(index);
    assert.strictEqual(value, "ok");
  });

  test("should produce a basic JS bundle with object rest spread support", async function () {
    const fixture = await Fixture.yaml/*yaml*/ `
      package.json:
        name: "javascript-object-rest-spread"
        type: "module"
        main: ./dist/index.js
    ...
    const res = output.default();
    assert.equal(res.y, "a");
    assert.deepEqual(res.z, { y: "a", b: "b" });
    assert.deepEqual(res.ys, { b: "b" });
  });

  test("should bundle node_modules for a browser environment", async function () {
    const fixture = await Fixture.yaml/*yaml*/ `
      package.json:
    ...
    const [entries, , bundles] = await fixture.bundle("main.js");
    ...
    const output = await evalEsmCb(entries.get("main.js"));

    assert.equal(typeof output, "function");
    assert.equal(output(), 3);
  });

  test("should not bundle node_modules for a node environment", async function () {
    const fixture = await Fixture.yaml/*yaml*/ `
      package.json:
        name: node_require
        type: commonjs
    ...
    await fs.promises.writeFile(
      fixture.path("node_modules", "test-module", "index.js"),
      "exports.a = 5;",
      "utf8",
    );

    const output = await evalCjsCb(entries.get("main.js"));
